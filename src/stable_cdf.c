...
void c_stable_cdf(double alpha, double beta, double sigma, double mu,
                         void *v_data, unsigned int row_count, void * v_cdf)
{
StableDist dist;
double * data = (double *) v_data;
double * cdf = (double *) v_cdf;
double *err = NULL;
double val;
int i,
    status;
...
// stable_cdf(dist, data, rowcount, pdf, err);

for (i=0; i < row_count; i++)
   {
   val=stable_cdf_point(&dist, data[i], err);
   cdf[i]=val;
   }
