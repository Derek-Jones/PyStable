...
void c_stable_rnd(double alpha, double beta, double sigma, double mu,
                         void *v_rnd, unsigned int n)
{
StableDist *dist;
double * rnd = (double *) v_rnd;

// puts("Here we go!");
/****
printf("alpha %f, beta %f, sigma %f, mu %f, n %i\n",
                alpha, beta, sigma, mu, n);
***/

dist=stable_create(alpha, beta, sigma, mu, 0);

stable_rnd(dist, rnd, n);
